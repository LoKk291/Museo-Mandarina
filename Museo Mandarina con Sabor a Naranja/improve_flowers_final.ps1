...
# Find the addFlowers method and replace it completely
...
addFlowers() {
        ...
        flowerGroup.position.y = 0.5; // Much lower - flowers start from inside vase
        ...
        if (this.flowerType === 'orchid') {
            ...
            // Brown center - larger and more detailed
            ...
            center.scale.set(1, 0.6, 1); // Flatten slightly
            stem.add(center);
            
            // Yellow petals - more realistic shape
            ...
            for (let i = 0; i < 16; i++) {
                ...
                petal.rotation.x = -0.3; // Tilt petals outward
                stem.add(petal);
            }
            ...
            for (let i = 0; i < 5; i++) {
                const leafGeo = new THREE.BoxGeometry(0.08, 0.9, 0.02);
                const leafMat = new THREE.MeshStandardMaterial({ 
                    color: 0x2d5016,
                    roughness: 0.6
                });
...
"@
        ...
        Write-Host "Updated flower positioning and improved sunflower design"
    }
}
